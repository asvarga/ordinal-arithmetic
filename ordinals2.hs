...
normal_nat :: Int -> ()
normal_nat 0 = normal (nat2ord' 0) === normal Zero *** QED
...
add' x Zero = x
add' Zero y = y
add' x@(Ord a0 n0 b0) y@(Ord a1 n1 b1) = case (comp a1 a0) of
    GT -> y
    LT -> (Ord a0 n0 (add' b0 y))
    EQ -> (Ord a0 (n0+n1) (add' b0 b1))
{-@ normal_add :: x:NFOrd -> y:NFOrd -> {normal (add' x y)} / [(size x), (size y)] @-}
